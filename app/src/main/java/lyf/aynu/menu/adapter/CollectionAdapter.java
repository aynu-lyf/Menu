...
import lyf.aynu.menu.view.SlidingButtonView;
...
public class CollectionAdapter extends RecyclerView.Adapter<CollectionAdapter.
    ...
    private List<DishBean> newsList = new ArrayList<>();
    private SlidingButtonView mMenu = null;
    public CollectionAdapter(Context context) {
        mContext = context;
        mIDeleteBtnClickListener = (IonSlidingViewClickListener) context;
    }
    public void setData(List<DishBean> newsList) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        DishBean bean = newsList.get(position);
        holder.tv_name.setText(bean.getTitle());
        holder.tv_newsTypeName.setText(bean.getImtro());
        Glide
                .with(mContext)
    ...
    public void removeData(int position, TextView tv_none, String userName) {
        DishBean bean = newsList.get(position);
        //从收藏新闻的数据库中也要删除此数据
        DBUtils.getInstance(mContext).delCollectionNewsInfo(bean.getId(), userName);
